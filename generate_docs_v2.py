# -*- coding: utf-8 -*-
"""
按分工差异化生成 6 个 docx 文件
- 3 个队员的《Python应用开发技术》期末大作业报告
- 3 个队员的 AI 参与说明
每个队员的内容根据其具体分工写出差异化版本
"""
import os
import sys
import io
...
def fill_assignment_for_leader(doc):
    """队长刘冰 - 重点：架构/爬虫/数据分析/可视化"""
    # 表 0：个人信息
    t0 = doc.tables[0]
    ...
    set_cell_text(t2.cell(2, 3), "artists 字段正确解析，多歌手用 / 分隔")
...
def fill_assignment_for_member_a(doc):
    """A 队员李强 - 重点：测试/导出/日志/异常/文档撰写"""
    t0 = doc.tables[0]
    set_cell_text(t0.cell(0, 1), PROJECT["title"])
...
def fill_assignment_for_member_b(doc):
    """B 队员张悦 - 重点：UI/按钮/词云/美化/打包"""
    t0 = doc.tables[0]
    set_cell_text(t0.cell(0, 1), PROJECT["title"])
    ...
    content = {
        ...
        "（基于数据分析结果，总结核心结论，示例：1. 某品类商品促销期间价格平均降幅达XX%；2. 销量与评价数量呈正相关关系等，300-500字）":
            ...
            "信息密度提升 5 倍以上；\n\n"
            "2. 柱状图精度最高：词云图只能看『谁更火』，柱状图能精确到「郑润泽 3 首 vs 周杰伦 2 首」的差异，" + "对决策类分析更友好；\n\n"
...
def fill_ai_for_leader(doc):
    """队长刘冰的 AI 参与说明 - 重点：爬虫/反爬/可视化"""
    info = doc.tables[0]
...
def fill_ai_for_member_a(doc):
    prompts = [
        ("爬取网易云热歌榜",
         ...
         "采纳", "运行后第一个端点返回 code=-1，逐步精简 headers 后改为只发送 User-Agent + Referer 才能成功"),
        ("解决 API 返回 code=-1",
         "网易云 playlist/detail API 返回 code=-1 是什么意思？",
         "AI 提示可能原因：参数错误、签名缺失、IP 被封、headers 异常",
         "部分采纳", "对比测试简单请求和 build_session 复杂请求，确认是 headers 过多触发压缩响应导致解析失败"),
        ("生成 matplotlib 中文柱状图",
         "matplotlib 画柱状图中文显示为方框，怎么解决？",
         "AI 提示需在 font_manager 中加载中文字体（simhei.ttf 或 msyh.ttc）",
         "采纳", "编写 find_chinese_font() 函数自动检测 Windows 系统字体路径，图表中文显示正常"),
    ]
...
def fill_ai_for_member_a(doc):
    ...
    prompts = [
        ("用 pytest 写单元测试",
         ...
         "采纳", "运行后发现 parametrize 用法报错，改为显式 for 循环 + assert，更易读"),
        ("CSV 导出 Excel 乱码",
         "Python 导出的 CSV 在 Excel 中打开中文乱码，怎么解决？",
         "AI 提示使用 utf-8-sig 编码（BOM 头）",
         "采纳", "在 generate_csv() 写入 \\ufeff 头部，Content-Type 改为 utf-8-sig，Excel/WPS 全部正常"),
        ("生成 Python 异常处理模板",
         "Python try/except 怎么写才能捕获所有异常并区分处理？",
         "AI 给出 except Exception as e 模式 + 4 种异常分类（网络/JSON/文件/空数据）",
         "部分采纳", "采纳了网络超时和 JSON 解析的处理，文件写入和空数据判断按业务实际调整"),
    ]
    for i, p in enumerate(prompts, 1):
        if i < len(pt.rows):
            for j, v in enumerate(p):
...
def fill_ai_for_member_b(doc):
    """B 队员张悦 - 重点：UI 设计/词云/PyInstaller 打包"""
    info = doc.tables[0]
    set_cell_text(info.cell(1, 1), "Python应用开发技术")
    set_cell_text(info.cell(2, 1), PROJECT["title"])
    set_cell_text(info.cell(3, 1), PROJECT["class_name"])
    ...
    prompts = [
        ("设计现代 Dashboard 风格",
         ...
         "采纳", "根据 AI 推荐确定了 CSS 变量系统，header 加上音频波形动画，整体观感更生动"),
        ("wordcloud 中文字体",
         "Python wordcloud 怎么设置中文字体？",
         "AI 提示 font_path 参数传入系统字体路径（如 C:/Windows/Fonts/simhei.ttf）",
         "采纳", "在代码中通过 os.path.exists() 检测多个字体路径，优先 simhei → msyh → simsun"),
        ("PyInstaller 打包 Flask 项目",
         ...
         "采纳", "首次打包时 templates 找不到导致 404，添加参数后正常；第二次打包 exe 双击闪退，"
...
def main():
    members = [
        {
            "name": "刘冰",
            "role": "队长",
            "asg_fn": fill_assignment_for_leader,
            "ai_fn": fill_ai_for_leader,
        },
        {
            "name": "李强",
            "role": "A队员",
            "asg_fn": fill_assignment_for_member_a,
            "ai_fn": fill_ai_for_member_a,
        },
        {
            "name": "张悦",
            "role": "B队员",
            "asg_fn": fill_assignment_for_member_b,
            "ai_fn": fill_ai_for_member_b,
        },
    ]
    ...
    for m in members:
        ...
        # 1) 大作业模版
        src1 = os.path.join(TEMPLATE_DIR, "2、大作业模版（新）.docx")
        doc1 = Document(src1)
        m["asg_fn"](doc1)
        out1 = os.path.join(OUTPUT_DIR, f"大作业报告_{m['role']}_{m['name']}.docx")
        doc1.save(out1)
        size1 = os.path.getsize(out1)
        print(f"  ✓ {out1} ({size1//1024}KB)")

        # 2) AI 参与说明
        src2 = os.path.join(TEMPLATE_DIR, "3、AI参与说明.docx")
        doc2 = Document(src2)
        m["ai_fn"](doc2)
        out2 = os.path.join(OUTPUT_DIR, f"AI参与说明_{m['role']}_{m['name']}.docx")
        doc2.save(out2)
        size2 = os.path.getsize(out2)
        print(f"  ✓ {out2} ({size2//1024}KB)")

    print()
    print("=" * 60)
    print(f"  ✓ 全部 6 个差异化文件已生成至: {OUTPUT_DIR}")
    print("=" * 60)
